...
# What is a website?
---
How did that work? Let’s familiarize ourselves with basics of each language by taking a peek at the different files that we used to make this website run!

There are three main languages that power the World Wide Web:

-   **HTML**, which stands for **H**yper**T**ext **M**arkup **L**anguage and is used to create the _structure and content_ of a website.
-   **CSS**, which stands for **C**ascading **S**tyle **S**heets. CSS gives a website _style_. If you’re interested in modifying colors, font types, font sizes, shadows, images, element positioning, and more, CSS is the tool for the job!
-   **JavaScript**, which can contain much more advanced logic than HTML and CSS, and is used to specify the _behavior and interactivity_ of web pages and web applications.
...
<html>
  <head>
    ...
    <script type="text/javascript" src="https://content.codecademy.com/courses/hour-of-code/js/alphabet.js"></script>
...
---
To do:
1. Let’s take a shot at modifying some HTML!
	
	In **index.html**, change the text in-between the `<h1>` tags. Instead of `Welcome to`, change the text to say `Introduction to`!
	
	Check out the hint if you need help!

2. HTML is very plain on its own! The beautiful websites that you see on the internet are styled with a variety of tools, including CSS.
	
	In the **styles.css** file, change the color property for the `<h1>` element from `black` to `teal`!

3. You’ll spend most of this Intensive learning JavaScript. Let’s take a look at the JavaScript code in **main.js**!
	
	Change the value of the `bubbleShape` variable on line 12 from `'circle'` to `'square'`.
	
	Press run and see the word change from circle bubbles to square bubbles!